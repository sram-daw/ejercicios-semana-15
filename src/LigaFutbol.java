...
public class LigaFutbol implements Liga {
    ...
    public LigaFutbol(String nombre) {
        ...
        this.vuelta = 1; //por defecto, todas las ligas que se creen empiezan con estos valores
    ...
    public List<AbstractMap.SimpleEntry<EquipoFutbol, EquipoFutbol>> simularJornada(ArrayList<EquipoFutbol> listaEquipos, List<AbstractMap.SimpleEntry<EquipoFutbol, EquipoFutbol>> parejasJornadas) {
        ...
        while (contadorPartidos < 12 && (this.jornadasFaltan > 21 || this.vuelta == 2)) { //la condición de las jornadas determina por qué vuelta va la ejecución y el || permite que siga entrando en el bucle cuando va por la segunda vuelta
            ...
            if (!isMismoEquipo && !(parejasJornadas.contains(new AbstractMap.SimpleEntry<>(listaEquipos.get(equipoRef), listaEquipos.get(equipoRival))) //comprueba que no existan parejas de equipos iguales en el AbstractMap para no repetir partidos
    ...
    public void calcularVueltas() {
        if (this.partidosFaltan <= 231) { //si va por la mitad de los partidos se pasa a la segunda vuelta
            this.vuelta = 2;
        }
    }
    ...
    public void calcularJornadasFaltan() {
        if (this.partidosFaltan % 11 == 0) { //solo se calculan las jornadas al final de cada una para evitar decimales
